  ...
  return (
    <section className="mt-24 px-6 md:px-12">
      <div className="max-w-6xl mx-auto">
        <h2 className="text-3xl md:text-4xl font-bold text-light_text text-center">Why InterviewAce?</h2>
        <p className="text-light_text text-center mt-4 max-w-2xl mx-auto">
        ...
        <div className="grid gap-8 mt-12 sm:grid-cols-2 lg:grid-cols-4">
          {features.map((f) => (
            <div key={f.title} className="rounded-lg border bg-white shadow-sm p-6 flex flex-col">
              ...
              <h3 className="mt-4 font-semibold text-light_text text-lg">{f.title}</h3>
              <p className="mt-2 text-sm text-light_text leading-relaxed">{f.description}</p>
            </div>
          ))}
        </div>
      </div>
